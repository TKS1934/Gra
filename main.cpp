...
int main(){
    ...
    sf::Clock clock;
    sf::Font font;
    if (!font.loadFromFile("arial.ttf")) { // Upewnij się, że plik arial.ttf jest w folderze projektu!
        cout << "Nie udalo sie zaladowac czcionki!" << endl;
    }
    sf::Text tekstGratulacje;
    tekstGratulacje.setFont(font); // To było kluczowe
            ...
            if (aktualnyStan == StanGry::Menu) {
                ...
                if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left){
                    if (tekstPowrot.getGlobalBounds().contains(mousePosF)) {
                        ...
                        gracz1= new gracz({10,20},{200,500},{0,0},window,grawitacja,0);
                    }
